...
class EmailSubscriber(models.Model):
    ...
    def confirm_url(self):
        ...
        relative_url = reverse('tinyblog_subscribe_confirm', args=[self.uuid_second, ])
        return u'http://{0}{1}'.format(current_site, relative_url)

    def unsubscribe_url(self):
        current_site = Site.objects.get_current()
        relative_url = reverse('tinyblog_unsubscribe', args=[self.uuid_second, ])
        return u'http://{0}{1}'.format(current_site, relative_url)


class PublishedPostManager(models.Manager):
    def get_query_set(self):
        return super(PublishedPostManager, self).get_query_set().filter(created__lte=datetime.now())
